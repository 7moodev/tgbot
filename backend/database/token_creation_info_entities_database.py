...
class TokenCreationInfoEntityDatabase(PostgresDatabase):
    ...
    def batch_insert(self, data_list: list[TokenCreationInfoEntity]):
        ...
        for data in data_list:
            ...
            query = f"""
                ...
                VALUES %s
                ON CONFLICT (tokenAddress) DO UPDATE
                SET {set_sql}
            """
    ...
    def fetch_by_address(self, address: str) -> TokenCreationInfoEntity:
        fetch_query = f"SELECT * FROM {self.table_name} WHERE tokenAddress = {address}"
        record = self.fetch_one(fetch_query)
        if (debug_should_log):
            logger.log(record)
        return record

tokenCreationInfoEntityDatabase = TokenCreationInfoEntityDatabase()
...
if __name__ == "__main__":
    try:
        db = TokenCreationInfoEntityDatabase()
