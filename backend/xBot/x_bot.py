...
async def get_potential_tokens():
    ...
    console.log('>>>> _ >>>> ~ filtered_by_volume:', [x['name'] for x in filtered_by_volume_and_mc])
    ...
    async with httpx.AsyncClient() as session:
        ...
        async def get_creation_info(token):
            token_creation_info = await birdeyeApiService.get_token_creation_info(token['address'])
            console.log('>>>> _ >>>> ~ token_creation_info:', token_creation_info)
            if token_creation_info['success'] and token_creation_info['data']['created'] > THRESHOLDS['created']:
                return token
            return None

        creation_info_tasks = [
            get_creation_info(token)
            for token in filtered_by_volume_and_mc
        ]
        filtered_by_time = [token for token in await asyncio.gather(*creation_info_tasks) if token]
        console.log('>>>> 1 >>>> ~ filtered_by_time:', filtered_by_time)
    console.log('>>>> 2 >>>> ~ filtered_by_time:', filtered_by_time)

    return
    # minimum # holders > 2000
        # via token_overview
    async with httpx.AsyncClient() as session:
        birdeyeApiService.with_session(session)
        async def get_token_overview(token):
            token_overview = await birdeyeApiService.get_token_overview(token['address'])
            if token_overview['success'] and token_overview['data']['holders'] > THRESHOLDS['holders']:
                return token
            return None

        overview_tasks = [get_token_overview(token) for token in filtered_by_time]
        filtered_by_holders = [token for token in await asyncio.gather(*overview_tasks) if token]
