...
class TokenDatabase(PostgresDatabase):
    ...
    def insert_token(self, token: TokenListEntity):
        insert_query = f"""
        INSERT INTO {self.table_name} (address, decimals, price, last_trade_unix_time, liquidity, logo_uri, mc, name, symbol, v24h_change_percent, v24h_usd)
    ...
    def fetch_token_by_address(self, address: str):
        fetch_query = f"SELECT * FROM {self.table_name} WHERE address = %s"
    ...
    def update_token(self, column_name: str, new_value, address: str):
        update_query = (
            f"UPDATE {self.table_name} SET {column_name} = %s WHERE address = %s"
        )
